...
const CONFIG_FILES: ConfigFileRow[] = [
  { id: 'board', route: '#/board', status: 'ready' },
  { id: 'features', route: '#/features', status: 'ready' },
  { id: 'blades', route: '#/blades', status: 'ready' },
  { id: 'bladeStyles', route: '#/styles', status: 'ready' },
  { id: 'presets', route: '#/presets', status: 'ready' },
];

export class PoHomePage extends PoElement {
  static styles = [poHostStyles, poPageStyles, poSectionCardStyles, poHomePageStyles];

  /**
   * Renders one table row describing a config file and its editor status.
   *
   * @param row Config file metadata including id, route link, and readiness status.
   * @returns Lit template for a single `<tr>` in the config files table.
   */
  private renderFileRow(row: ConfigFileRow) {
    const pathKey = homePageConfigFilePathKeys[row.id];
    const pathLabel = homePageI18n.translate(pathKey);
    return html`
      <tr>
        <td class="file-path">
          <a
            class="file-link"
            data-testid="home-page-file-link-${row.id}"
            href=${row.route}
            >${pathLabel}</a
          >
        </td>
  ...
  render() {
    return html`
      <section class="page" data-testid="home-page">
        ...
        <wa-card class="section-card">
          <h3 class="section-title">${homePageI18n.translate(homePageKeys.cardWhatItDoesTitle)}</h3>
          <p class="section-lead">${homePageI18n.translate(homePageKeys.cardWhatItDoesBody)}</p>
          <p class="hint">${homePageI18n.translate(homePageKeys.cardWhatItDoesHint)}</p>
        </wa-card>

        <div class="hub-grid" data-testid="home-page-hub-grid">
          <wa-card class="section-card">
            <h3 class="section-title">${homePageI18n.translate(homePageKeys.cardConfigFilesTitle)}</h3>
          ...
          <wa-card class="section-card">
            <h3 class="section-title">${homePageI18n.translate(homePageKeys.cardWorkflowTitle)}</h3>
            <ol class="steps">
