...
|-----------|-------------|-------------------|
| SmokeBlade | `[smoke_blade]` | **`solid`** base + **`smoke_flow`** + **`stripes`** multiply + **`swing`** add + OS7-style combat overlays |
| Smoke laser (SD variant) | `[smoke_laser]` | Same **`smoke_flow`** stack; green base + **`random_bands`** (not **`stripes`**) + classic **`clash`** / **`blast`** |
| Sine / smoke sine (SD demos) | `[sine_waves_cyan]` / `[smoke_sine_cyan]` | **`sine_waves`** multiply masks + OS7 combat; latter adds **`smoke_flow`** on **`solid_bend`** |
